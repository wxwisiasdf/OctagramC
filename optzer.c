...
static void cc_optimizer_expr_condense_binop(
    cc_context* ctx, cc_ast_node** pnode, bool managed)
{
    cc_ast_node* node = *pnode;
    assert(node != NULL);
...
static void cc_optimizer_expr_condense_block(
    cc_context* ctx, cc_ast_node** pnode, bool managed)
{
    cc_ast_node* node = *pnode;
    size_t i;
...
static void cc_optimizer_expr_condense_call(
    cc_context* ctx, cc_ast_node** pnode, bool managed)
{
    cc_ast_node* node = *pnode;
    size_t i;
...
/* Reduces a redundant unary operation, for example:
   &*arr -> &arr
   Or &*&*&*arr -> &arr
*/
bool cc_optimizer_reduce_redundant_unop(
    cc_context* ctx, cc_ast_node** pnode, bool managed)
{
    return true;
}
    ...
    cc_context* ctx, cc_ast_node** pnode, bool managed)
{
    ...
    switch (node->type) {
    case AST_NODE_BINOP:
        cc_optimizer_expr_condense_binop(ctx, pnode, managed);
        break;
    case AST_NODE_BLOCK:
        cc_optimizer_expr_condense_block(ctx, pnode, managed);
        break;
    case AST_NODE_CALL:
        cc_optimizer_expr_condense_call(ctx, pnode, managed);
        break;
    case AST_NODE_IF:
